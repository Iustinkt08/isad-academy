 ...
 * Draft courses 404 here exactly as on the page (public access, no user). Any render
 * failure logs a `[course-pdf]` breadcrumb on STDERR (stdout is lost under Passenger)
 * and answers 500 without taking the page down.
 */
...
export async function GET(request: Request, { params }: Args): Promise<Response> {
  ...
  try {
    ...
    console.error(`[course-pdf] generarea a eșuat pentru "${slug}":`, error)
